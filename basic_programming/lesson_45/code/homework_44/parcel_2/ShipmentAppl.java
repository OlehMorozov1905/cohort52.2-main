...
public class ShipmentAppl {
    public static void main(String[] args) {
        ...
        int totalParcels = containers.stream().mapToInt(Container::countParcels).sum();
        ...
        Container maxParcelContainer = containers.stream()
                .max((c1, c2) -> Integer.compare(c1.countParcels(), c2.countParcels()))
                .orElse(null);
        System.out.println("---------------------------------------------------");
        ...
        List<Container> containersWithMaxParcelCount = containers.stream()
                .filter(c -> c.countParcels() == maxParcelContainer.countParcels())
                .toList();
